...
describe('application.start(app, port = 3000)', () => {
	...
	it('should log startup only after the server is listening', () => {
		const app = express();
		const server = new EventEmitter();
		server.once = server.on.bind(server);
		...
		expect(mockInfo).toHaveBeenCalledWith('Application started. Visit: http://localhost:9000.');
	});

	it('should log server start errors', () => {
		const app = express();
		const server = new EventEmitter();
		server.once = server.on.bind(server);
		app.listen.mockReturnValue(server);
		...
		expect(mockError).toHaveBeenCalledWith({ err }, 'Application failed to start on port 9000.');
	});
});
