...
describe('middlewares.cors()', () => {
	const { NODE_ENV, WHITELIST } = process.env;
	...
	it('should call cors with function as arg if WHITELIST env if defined', () => {
		process.env.WHITELIST = 'http://example1.com, http://example2.com';
		middlewares.enableCors();
		expect(cors).toHaveBeenCalled();
		expect(mockwarn).not.toHaveBeenCalled();
		expect(cors).toHaveBeenCalledWith({ origin: expect.any(Function) });
	});
