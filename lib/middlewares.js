...
const formatValidationErrors = (validationErrors) => Object.entries(validationErrors)
	.reduce((acc, [reqProp, errors]) => {
		acc[reqProp] = errors.map((error) => {
			const formattedError = { ...error };
			delete formattedError.message;
			if (!formattedError.dataPath) delete formattedError.dataPath;
			return formattedError;
		});
		return acc;
	}, {});

const defaultCustomFn = () => false;

const logMiddleware = ({
	ignorePaths = ['/healthy'],
	logger = log(),
	customReqSerializer = defaultCustomFn,
	customResSerializer = defaultCustomFn,
} = {}) => {
	...
	const errSerializer = (err) => {
		const isErr4xx = err.raw && err.raw.isBoom
	...
	const reqSerializer = (req) => customReqSerializer(req) || {
		id: req.id,
	...
	const resSerializer = (res) => customResSerializer(res) || {
		headers: {
