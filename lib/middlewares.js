...
const forceHttps = (port = 443) => (req, res, next) => {
	...
	const hostname = req.hostname || (() => {
		...
		if (host.startsWith('[')) {
			...
			return hostEnd === -1 ? host : host.slice(0, hostEnd + 1);
		}
		return host.split(':')[0];
	})();
	const hostnamePort = port !== 443 ? `:${port}` : '';
	res.redirect(301, `https://${hostname}${hostnamePort}${req.originalUrl}`);
};
...
const formatValidationErrors = (validationErrors) => Object.entries(validationErrors)
	.reduce((acc, [reqProp, errors]) => {
		acc[reqProp] = errors.map((error) => {
			...
			if (!formattedError.instancePath) delete formattedError.instancePath;
