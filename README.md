...
A backbone to create Web and REST servers.  
...
```js
// server.js
const { application } = require('apitools');
...
```js
// server.js
const { application, config } = require('apitools');
// ...
...
### Request validation
```js
const { application, validator } = require('apitools');
...
```js
const { application, asyncWrapper } = require('apitools');
...
**In production:**
```sh
NODE_ENV=production apitools dist
```
...
When you require apitools, it reads the environment variables defined in .env files using dotenv.  
...
```js
// server.js
const { config, log() } = require('apitools');
...
Example:
```js
const { middlewares } = require('apitools');
...
Example:  
```js
const { validator } = require('apitools');
app.get(
	'/valid',
	validator().validate({
		query: {
...
Example:
```js
const { asyncWrapper } = require('apitools');
// ...
app.get(
	'/persons',
	asyncWrapper(async (req, res, next) => { await Person.findAll(); }),
...
...
"scripts": {
	"debug": "apitools debug",
	"dist": "apitools dist",
	"start": "apitools start",
	"test": "apitools test", 
}
...
```
